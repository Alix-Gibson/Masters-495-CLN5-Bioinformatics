#!/usr/bin/env python3
# Python 3 compatibility port of MAGeCK-iNC v5.2.
# NOTE: This is the first-pass compatibility conversion.
# The statistical algorithm has intentionally not been rewritten.
...
INPUT_FOLDER = "/projects/health_sciences/bms/biochemistry/hughes_lab/AlixGibson/MAGeCK_iNC/Filtered_Diff20/Results/R2"

OUTPUT_FOLDER = "/projects/health_sciences/bms/biochemistry/hughes_lab/AlixGibson/MAGeCK_iNC/Filtered_Diff20/Results/R2/iNC"
...
if GENE_LIST is None:
    ...
    with open(GENE_LIST) as f:
        genes_to_label = [i.strip() for i in f]
def process_comparison(comparison):
    ...
    df.sort_values('product',ascending=False).to_csv(comparison_folder + '/' + comparison + '_all_genes.csv',index=False)
    df_hits.sort_values('product',ascending=False).to_csv(comparison_folder + '/' + comparison + '_fdr%s_product%s_hits.csv'%(FDR, thres),index=False)
    ...
    if MAKE_PLOT:
        ...
        plt.savefig(comparison_folder + "/" + comparison + '_volcano_plot.pdf')
        plt.show()
